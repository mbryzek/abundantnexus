#!/usr/bin/env bash
#
# What CI runs for abundantnexus.
#
...
#
#   dev ci verify --repo mbryzek/abundantnexus --sha <head sha> --pr <n> --no-post
#
...
echo "building ${CI_REPO:-abundantnexus} @ ${CI_SHA:-working tree} (${CI_EVENT:-local}, clean=${CI_CLEAN_BUILD:-?})"
...
# svelte-check + eslint --max-warnings 0 + prettier --check. There is no unit
# suite and no Playwright suite in this repo yet — when one lands it goes inside
# `check` as `test:unit`, not as a line here.
# `npm run check` is the whole gate in every SvelteKit repo in this fleet
# (ISS-3885), so there is deliberately no second test step below.
npm run check
