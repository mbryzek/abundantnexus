	...
	<div class="presentations-table-wrapper">
		<table class="presentations-table" aria-label="Conference presentations with video and audio recordings">
			<caption class="sr-only">
				Video and audio recordings of all Abundant Nexus Conference presentations.
			</caption>
			<thead>
				<tr>
					<th>Video</th>
			...
			<tbody>
				{#each presentations as p}
					<tr>
						<td>
							<a
								class="media-link mp4"
								href={p.videoUrl}
								target="_blank"
								rel="noopener noreferrer"
								aria-label="Watch video: {p.topic}"
							>
								MP4
							</a>
						</td>
						<td>
							<a
								class="media-link mp3"
								href={p.audioUrl}
								target="_blank"
								rel="noopener noreferrer"
								aria-label="Listen to audio: {p.topic}"
							>
								MP3
							</a>
						</td>
						...
						<td class="topic">
							{#if p.isKeynote}<strong>Keynote:</strong> {/if}{p.topic}
						</td>
						<td>
